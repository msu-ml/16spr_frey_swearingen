...
This document includes the a near-final draft of those sections.
...
It will also be interesting to see how the performance changes as the data size grows.

There are three primary parameters that we vary in our synthetic dataset.
The first parameter is the number of clusters ($N_C$) where we use 5 different values: 100, 1,000, 10,000, 100,000, and 1,000,000.
The second parameter is the number of features ($N_F$)  where we use 3 different values: 100, 1,000, and 10,000.
The third and final parameter is the number of samples per cluster ($N_S$) where we use 4 different values: 100, 1,000, 10,000, and 100,000.
...
\begin{tabular}{|c||c||c|}
\hline
\textbf{Number of Clusters} & \textbf{Number of Features} & \textbf{Number of Samples Per Cluster} \\
\hline
100 & 100 & 100 \\
1,000 & 1,000 & 1,000 \\
...
An overview of this method is shown in Figure \ref{fig:meangeneration}.
We first generate a set of $N_C$ means.
We don't want the clustering to be too simple so we introduce an average mean euclidean distance parameter ($\mu_\text{euc}$).
...
To demonstrate the data we ran the above procedure with $N_C = 5$, $N_F = 2$, and $N_S = 100$.
Since the data is two dimensional, we can visualize it.
Two examples are shown in Figure \ref{fig:dataexamples}.

\begin{figure}[!htbp]
\centering
\subfloat[Example Data]{\includegraphics[width = 0.5\linewidth]{figures/RandDataViz.eps}}
~
\subfloat[Example Data]{\includegraphics[width = 0.5\linewidth]{figures/RandDataViz2.eps}}
\caption{Examples of syntethic data generation. Each cluster is represented with different colors.}
