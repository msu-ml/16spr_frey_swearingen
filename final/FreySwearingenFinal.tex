...
We define parameters used in the Yinyang K-Means method here.
...
We know that $ub \func{x}$ is the distance between $x$ and it's assigned cluster $b \func{x}$.
...
The right-hand side of Equation~\ref{eq:gfc}, $ub \func{x} + \delta \func{b \func{x}}$, tell us the distance between $x$ and the location of $\delta \func{b \func{x}}$ at iteration $i+1$.
...
The $\delta \func{c}$ term is the distance $x$'s assigned cluster at iteration $i$ moves between iteration $i$ and $i+1$.
Thus, the right-hand side of Equation \ref{eq:lfc} represents the closest possible distance a center in $G_i'$ other than $b \func{x}$ could possible be in iteration $i+1$.
%Therefore, the Local-Filtering Condition test where it's possible a cluster center outside of $G_i'$ is becomes the closest cluster center for a point in $G_i$ during the cluster update step between iteration $i$ and iteration in $i+1$.
%The Local-Filtering Condition accounts for the \textit{intergroup} interactions among data points and clusters.
...
\end{equation*}
This theorem stems from the fact that $lb \func{x, G_i} + \delta \func{c}$ represents the minimum distance between $x$ and the closest center in $G_i'$ other than $c = b \func{x}$.
