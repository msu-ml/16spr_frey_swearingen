...
However, the GraphLab was unable to download this package in it's configuration process.
...
This seemed to become lost cause, so we next tried to download the latest version of GraphLab and use it with the \texttt{yykmeans.cpp} distributed by the authors.
However, since the time that the paper was presented in ICML in the Summer of 2015, it appears GraphLab was had a significant change to it's API.
...
In order to make a more fair comparison we implemented our own K-means algorithm to avoid any possible under-the-hood optimizations that may exists in the native MATLAB version.
...
In the author's work, they achieved a speed-up regardless of data set size, number of features, or number of clusters.
For these reasons, Yinyang K-means offers an improved clustering method.

\section{Future Work}
The most obvious step is to ensure our implementation matches the author's exactly.
...
Ben implemented the algorithm in MATLAB and corresponded with the original authors when our results didn't match theirs.
